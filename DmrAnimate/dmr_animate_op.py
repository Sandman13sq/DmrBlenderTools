...
class DMRANIM_OP_MakeControl(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        kbdatamap = {} # <keyname: (obj, keyblock, shapekeys)>
        keynames = [];
        ...
        for meshobj in meshobjects:
            ...
            for kb in shapekeys.key_blocks[1:]:
                if kb.name[0] == '!':
                    continue
                if kb.name not in kbdatamap.keys():
                    kbdatamap[kb.name] = [];
                    keynames.append(kb.name);
                kbdatamap[kb.name].append( (meshobj, kb, shapekeys) )
        ...
        for b in [b for b in bones if b.name not in kbdatamap.keys()]:
            b.select = True
        bpy.ops.object.mode_set(mode='OBJECT')
        bpy.ops.object.mode_set(mode='EDIT')
        ...
        z = r*2 if len(kbdatamap) > 1 else r*4
        for i, k in enumerate(keynames):
            ...
            ebone.tail = (0.0, r, z)
            pbone = posebones[k]
            pbone.location[0] = kblist[0][1].value;
            
            z += r*2
            if i == len(kbdatamap) // 2 - 1:
                z += r*2
        ...
        for kbentry in [x for y in kbdatamap.values() for x in y]: # Clear all drivers
            shapekeys = kbentry[2]
        ...
        # Add new drivers
        for keyname, kblist in kbdatamap.items():
            for shapekeys in [x[2] for x in kblist]:
                datapath = 'key_blocks["%s"].value' % keyname
                ...
                v.targets[0].bone_target = keyname
...
class DMRANIM_OP_ToggleSDS(bpy.types.Operator):
    bl_label = "Toggle Subsurf"
    bl_idname = 'dmr.toggle_subsurf';
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        for obj in context.scene.objects:
            ...
            if obj.modifiers:
                for m in obj.modifiers:
                    if m.type == 'SUBSURF' and m.name[0] != '!':
                        targetmodifiers.append(m);
                        if m.levels > 1:
                            allones = False;
        
        if allones:
            for m in targetmodifiers:
                m.levels = 2;
            self.report({'INFO'}, 'All subsurfs set to 2')
        else:
            for m in targetmodifiers:
                m.levels = 1;
            self.report({'INFO'}, 'All subsurfs set to 1')
        
        return {'FINISHED'}

classlist.append(DMRANIM_OP_ToggleSDS);
